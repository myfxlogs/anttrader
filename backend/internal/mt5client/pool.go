...
func NewConnectionPool(cfg *config.MT5Config, maxSize int) *ConnectionPool {
	return &ConnectionPool{
		cfg:     cfg,
		maxSize: maxSize,
		pool:    make([]*MT5Connection, 0, maxSize),
		client:  NewMT5Client(cfg),
	}
}

func (p *ConnectionPool) Get(ctx context.Context, user uint64, password, host string, port int32) (*MT5Connection, error) {
	p.poolMu.Lock()
	if p.closed {
		p.poolMu.Unlock()
...
func (p *ConnectionPool) createConnection(ctx context.Context, user uint64, password, host string, port int32) (*MT5Connection, error) {
	return p.client.Connect(ctx, user, password, host, port)
}

func (p *ConnectionPool) Return(conn *MT5Connection) {
	if conn == nil {
		return
	}
...
func NewPoolManager(cfg *config.MT5Config) *PoolManager {
	pm := &PoolManager{
		cfg:     cfg,
...
func (pm *PoolManager) GetConnection(ctx context.Context, broker string, user uint64, password, host string, port int32) (*MT5Connection, error) {
	pool := pm.GetPool(broker)
	return pool.Get(ctx, user, password, host, port)
}

func (pm *PoolManager) ReturnConnection(broker string, conn *MT5Connection) {
	if conn == nil {
		return
	}
	pool := pm.GetPool(broker)
